export default function Searchbar({ callback } : { callback: (data: string) => void}) {
    const submitHandler = (e : React.FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        const formData = new FormData(e.currentTarget)
        callback(formData.get('search') as string)
    }
    return (
    <form className="relative" onSubmit={submitHandler}>   
        ...
        <div className="relative">
            ...
            <input type="search" id="search" name="search" className="block w-full p-4 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Buscar productos..." required/>
